...
//	glortho(10);	
	...
	unsigned shader_program = 
		make_shader_program(read_file("src/vertex_shader.glsl"), 
				read_file("src/fragment_shader.glsl"), "a_pos");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		auto tm = m.glscale(0.2,0.2,0.1) * KeyBindMatrix * m.glrotateX(k) * m.gltranslate(0,3,0) * m.glscale(0.5, 3, 0.5);
